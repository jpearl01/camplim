...
proto._setMany = function(fields, options) { // was (fieldName, setValue, options)
  var doc = this;
  ...
  _.each(fields, function(setValue, fieldName) {
...
proto._setOne = function(fieldName, setValue, options) {
  ...
  // EDIT: let's avoid spamming events and do these at once
  //
  // there's a lot of work in _setOne()
  // right now, _setMany() uses _setOne() to get it all done one at a time
  // let's invert that and have _setOne() use _setMany() to get it done.
  // that way, we can handle one via a many with only one,
  // and,
  // we can handle many as a single operation. way less events.
  ...
  // only do this if setValue is defined
  if (setValue) {
    fieldValues = {}
    fieldValues[fieldName] = setValue
    doc._setMany(fieldValues, options)
  }
...
proto.set = function(/* arguments */) {
  var doc = this;
  // this doesn't allow an options object as a second arg when
  // setting multiple values via an object as the first arg...
  // it actually will do *nothing* if that happens...
  //
  // flip this to test the type of the args and adapt..
  // (object, object) is _setMany with options
  // (object) is _setMany without options
  // (string, object) is _setOne with an object being the value, not options
